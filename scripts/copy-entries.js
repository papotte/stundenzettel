...
async function copyUserData(sourceUserId, targetUserId) {
  try {
    const sourceCollection = db.collection('users').doc(sourceUserId).collection('timeEntries')
    const targetCollection = db.collection('users').doc(targetUserId).collection('timeEntries')
