...
test.describe('Export Page', () => {
  ...
  test('should allow editing an entry from the export page and reflect changes', async ({ page }) => {
    // Add an entry
    ...
    const entryCell = page.getByTestId(`timesheet-day-${format(new Date(), 'yyyy-MM-dd')}`);
  ...
  test('should allow cancelling edit in export page without saving changes', async ({ page }) => {
    // Add an entry
  ...
  test('should allow adding a new entry to a day from the export page', async ({ page }) => {
    await page.getByRole('link', { name: 'Vorschau & Export' }).click();
    await page.waitForURL('/export');
    ...
    const addButton = dayRow.getByRole('button', { name: /Add entry|Hinzufügen/ });
