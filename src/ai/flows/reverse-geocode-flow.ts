...
const reverseGeocodeFlow = ai.defineFlow(
  ...
  async ({ latitude, longitude }) => {
    ...
    try {
      ...
      const locality = getAddressComponent(bestResult.address_components, 'locality');
      const town = getAddressComponent(bestResult.address_components, 'postal_town');
      const city = getAddressComponent(bestResult.address_components, 'administrative_area_level_2');

      // Use the first available, from most to least specific, with a fallback to the first part of the formatted address.
      const address = locality || town || city || bestResult.formatted_address.split(',')[0];
